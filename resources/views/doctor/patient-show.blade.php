...
<div style="display:flex;align-items:center;gap:12px;margin-bottom:1.25rem">
    @if($mp->image)
    <img src="{{ Storage::url($mp->image) }}" alt="" style="width:48px;height:48px;border-radius:50%;object-fit:cover">
    @else
    <div class="avatar" style="width:48px;height:48px;background:#E1F5EE;color:#0F6E56;font-size:18px">{{ substr($member->name, 0, 1) }}</div>
    @endif
    <div><div style="font-size:20px;font-weight:500">{{ $member->name }}</div><div style="font-size:12px;color:var(--color-text-secondary)">{{ $mp->goal ?? '—' }} • {{ $mp->current_weight ?? '?' }} كغ</div></div>
    <div style="margin-right:auto;display:flex;gap:6px">
        <a href="{{ route('chat.index') }}" class="btn btn-primary" style="font-size:12px"><i class="ti ti-message"></i> رسالة</a>
        <a href="{{ route('doctor.nutrition.create', $member->id) }}" class="btn" style="font-size:12px"><i class="ti ti-salad"></i> غذائي</a>
        <a href="{{ route('doctor.workout.create', $member->id) }}" class="btn" style="font-size:12px"><i class="ti ti-barbell"></i> تمرين</a>
    </div>
</div>
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:1rem">
    <div class="card" style="padding:10px;text-align:center"><div style="font-size:22px;font-weight:500;color:#1D9E75">{{ $mp->current_weight ?? '—' }} كغ</div><div style="font-size:11px;color:var(--color-text-secondary)">الوزن</div></div>
    <div class="card" style="padding:10px;text-align:center"><div style="font-size:22px;font-weight:500;color:#534AB7">{{ $mp->body_fat ?? '—' }}%</div><div style="font-size:11px;color:var(--color-text-secondary)">الدهون</div></div>
    <div class="card" style="padding:10px;text-align:center"><div style="font-size:22px;font-weight:500;color:#185FA5">{{ $mp->progress_percentage ?? 0 }}%</div><div style="font-size:11px;color:var(--color-text-secondary)">التقدم</div></div>
    <div class="card" style="padding:10px;text-align:center"><div style="font-size:22px;font-weight:500;color:#854F0B">★★★★★</div><div style="font-size:11px;color:var(--color-text-secondary)">التزام</div></div>
</div>

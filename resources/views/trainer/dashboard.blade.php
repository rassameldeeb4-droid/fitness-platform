...
<div>
<div class="card" style="padding:10px 14px;display:flex;gap:8px;align-items:center;margin-bottom:1rem">
    ...
    <input placeholder="شارك تحديثاً مع متدربيك..." style="flex:1;border:0.5px solid var(--color-border-secondary);border-radius:20px;padding:8px 14px;font-size:13px;background:var(--color-background-primary);color:var(--color-text-primary);font-family:var(--font-sans);outline:none">
...
<div class="card">
    ...
    @php $sessions = [['9:00','أحمد الغامدي','قيد الإنجاز'],['11:00','نورا السلمي','قادمة'],['2:00','فيصل المطيري','قادمة'],['5:00','ريم الشهري','قادمة']]; @endphp
    ...
    <div style="display:flex;gap:8px;align-items:center;padding:6px 0;border-bottom:0.5px solid var(--color-border-tertiary);font-size:12px">
        <span style="color:#1D9E75;font-weight:500;min-width:35px">{{ $s[0] }}</span>
        <span style="flex:1">{{ $s[1] }}</span>
        <span class="badge {{ $s[2]=='قيد الإنجاز' ? 'badge-green' : 'badge-blue' }}">{{ $s[2] }}</span>
    </div>
    @endforeach
</div>
</div>

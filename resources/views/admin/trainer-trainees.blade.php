@extends('layouts.app')
@section('title', 'متدربو ' . $trainer->name)
@section('content')
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.25rem">
    <div class="page-title" style="margin-bottom:0">متدربو <strong>{{ $trainer->name }}</strong></div>
    <a href="{{ route('admin.trainers.show', $trainer->id) }}" class="btn"><i class="ti ti-arrow-right"></i> عودة</a>
</div>
@if($trainees->count())
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:10px">
    @php $colors = [['bg'=>'#E1F5EE','tc'=>'#0F6E56'],['bg'=>'#FAEEDA','tc'=>'#854F0B'],['bg'=>'#EEEDFE','tc'=>'#3C3489'],['bg'=>'#E6F1FB','tc'=>'#185FA5']]; @endphp
    @foreach($trainees as $i => $p)
...
<div class="card" style="text-align:center;padding:2rem">
    ...
    <p style="color:var(--color-text-secondary)">لا يوجد متدربين لهذا المدرب</p>
</div>
@endif
@endsection
